...
- **SignalK delta shape is the data contract.** Three client modes in `src/signalk/client.ts`, switched via `VITE_SIGNALK_MODE`:
  ...
  - **Supplementary internet AIS (geo/real only, never `mock`)** — `src/signalk/aisStream.ts` + `src/ais/useInternetAis.ts` relay shore-station AIS from aisstream.io when enabled in Settings (operator's own free API key; needs cellular data). Real but **delayed and coverage-gapped**: vessels are flagged `relayed`, capped at `monitor` threat band (a possibly-minutes-old position must never fire a collision warning), labeled "via shore relay", and the AIS list shows relay connection status so an empty list while offline doesn't read as "no traffic". Wire units (knots/degrees) convert to SignalK SI at the source; a direct receiver report for the same vessel clears the flag. A refused subscription (bad API key) surfaces as a distinct `rejected` status — never mislabeled as no-signal — and stops reconnect churn until Settings change. **Any external endpoint the app talks to must be in the CSP `connect-src` allowlist in `vite.config.ts`** — the built app silently blocks everything else (this bit the aisstream relay once: the socket never left the phone and the UI blamed cell signal).
- **PWA updates are prompt-mode, never auto-reload** (`registerType: 'prompt'` + `src/pwa/UpdatePill.tsx`): a new version downloads in the background and WAITS; the pill offers it and the operator applies it — autoUpdate restarted the app ~10s after launch, mid-read or mid-passage. A cold start also applies a waiting update.
...
**Phone chrome compaction (max-width 700px / max-height 500px):** the boat nameplate and POSITION/SPEED/HEADING labels go screen-reader-only, metric fonts step down from helm scale, tabs drop to a 44px min-width (from 56px) so the full control row holds one line at 375px, and the ?/collapse buttons dock to the right edge of the metrics block instead of taking their own line. **The bar's swipe-collapse gesture ignores touches originating inside an open slide panel** — Settings/MOB/theme panels render inside the header, so a panel scroll bubbled to the collapse handler, collapsed the bar, and UNMOUNTED the open panel mid-read (found 2026-07-19 after multiple on-water reports; e2e-guarded in statusbar.spec). StatusBar chrome measured 330px tall at 390px wide before this pass; ~190px after. Verify with Playwright screenshots when touching StatusBar styles.

**Help:** the `?` button in the StatusBar metrics row (all viewports, visible even with controls collapsed) opens `src/help/HelpContent.tsx` — the app's full plain-language manual, also reachable from Settings. It documents every feature; keep it in sync when shipping user-visible behavior. It has a Close button at top AND bottom, plus SlidePanel's pinned ✕. **Slide panels have NO swipe-to-close gesture** — two implementations (whole-panel y-range, then a touch-action:none handle strip) both misread real-iOS scrolls as dismissals; dismissal is the sticky ✕, Escape, tap-outside, and each panel's own buttons. Panels always open pinned to the top (scrollTop reset + preventScroll focus).
...
The StatusBar's left section includes a glanceable time + sun + tide cluster (`src/statusbar/ClockSunTide.tsx`). Format: `2:32 PM · ☀↘ 7:47 PM · 〰↗ High 4:15 PM`. The station NAME is deliberately not shown in the bar (decision 2026-07-19 — it crowded the phone); it lives in the tide tooltip, the sr-only text, and a read-only "Tide station" section in Settings.
...
- **Tide** — pre-fetched NOAA hi/lo predictions for Bar Harbor / Castine / Rockland / North Haven / Vinalhaven. The canonical station list lives in `CANONICAL_STATIONS` (`src/utils/useTideRefresh.ts`) — adding a station there is enough: the refresh hook detects the loaded data is missing it and pulls predictions next time the device is online (also mirror it in `scripts/fetch-tide-predictions.mjs` so fresh installs have it offline), shipped as `public/tides/<year>.json` and refreshed in the background by `useTideRefresh` when the device sees a network. Continuous water level via cosine interpolation between bracketing events; `nearestStation(pos)` picks the reference. M2 stub remains as a last-resort fallback when both IDB and the bundle are missing — UI dims and prefixes the pill with `~` in that case. Refresh annually with `node scripts/fetch-tide-predictions.mjs`. See [docs/tides.md](docs/tides.md).
